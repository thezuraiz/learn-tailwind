...
const Nav = () => {
  ...
  return (
    <header
      className={`absolutes fixed top-0 w-full z-20 py-8 padding-x transition-all delay-100 ease-in-out ${
      ...
      <nav className="flex justify-between items-center max-container">
        ...
        <ul className="flex flex-1 justify-center items-center max-container gap-16 max-lg:hidden">
          {navLinks.map((e) => (
            <li key={e.label}>
              <a
                className="font-montserrat leading-normal text-lg text-slate-gray hover:text-coral-red transition-all ease-in-out duration-200"
                href={e.href}
              >
                {e.label}
              </a>
